...
Riyadh Taraweeh is a Flask + React web application serving as a directory of Taraweeh prayer imams and mosques in Riyadh, Saudi Arabia. Users can search, filter by district, sort by proximity, listen to imam audio samples, and bookmark favorites.
...
- API routes unchanged: `/api/mosques`, `/api/mosques/search`, `/api/mosques/nearby`
- Admin routes still use Jinja2: `/admin/`, `/login`

### Frontend (`frontend/src/`)

```
src/
├── App.tsx              # Providers (Query, Audio, Favorites) + Routes
├── index.css            # Tailwind + CSS variables + animations
├── lib/
│   ├── api.ts           # fetchMosques, searchMosques, etc.
│   └── arabic-utils.ts  # normalizeArabic, formatDistance
├── hooks/
...
│   └── FavoritesContext.tsx # localStorage persistence
├── components/
...
- **Mosque:** name, location, area (district), map_link, latitude, longitude
...
These are replaced by React but kept as backup:
- `templates/index.html`, `base.html`, `about.html`, `contact.html`, `mosque_detail.html`
- `static/css/styles.css`
- `static/js/script.js`

See `FRONTEND_MIGRATION.md` for full migration details.
